...
(assert (equal 75.0 (f-h 'Nantes *heuristic*))) ;;  75.0 
(assert (equal 145.0 (f-h 'Marseille *heuristic*))) ;; 145.0
(assert (equal 105.0 (f-h 'Lyon *heuristic*))) ;; 105.0
(assert (equal NIL (f-h 'Madrid *heuristic*)))  ;; NIL
(assert (equal 50.0 (f-h 'Nancy *heuristic*))) ;; 50.0
(print 'EXERCISE_1_OK)

(assert (equal 30.0 (action-cost (car (navigate 'Avignon *trains*))))) ;; 30.0
(assert (equal '(30.0 16.0) (mapcar #'action-cost (navigate 'Avignon *trains*)))) ;; (30.0 16.0)
(assert (equal 'LYON (action-final (car (navigate 'Avignon *trains*))))) ;; LYON
(assert (equal  '(CALAIS NANCY NEVERS ORLEANS ST-MALO) (mapcar #'action-final (navigate 'Paris *trains*)))) ;; (CALAIS NANCY NEVERS ORLEANS ST-MALO)
(print 'EXERCISE_2_OK)

(assert (equal NIL (f-goal-test node-calais '(Calais Marseille) '(Paris Limoges)))) ;; NIL
(assert (equal NIL (f-goal-test node-paris '(Calais Marseille) '(Paris)))) ;; NIL
(assert (equal T (f-goal-test node-calais '(Calais Marseille) '(Paris Nancy)))) ;; T
(assert (equal T (f-goal-test node-calais '(Calais Marseille) '()))) ;; T
(print 'EXERCISE_3_OK)

(assert (equal T (f-search-state-equal node-calais node-calais-2 '()))) ;; T
;(assert (equal NIL (f-search-state-equal node-calais node-calais-2 '(Reims))) ;; NIL
;(assert (equal T (f-search-state-equal node-calais node-calais-2 '(Nevers)))) ;; T
(assert (equal NIL (f-search-state-equal node-nancy node-paris '()))) ;; NIL
;(assert (equal T (f-search-state-equal node-calais node-calais '(Paris Nancy)))) ;; T
(print 'EXERCISE_4_OK)

(assert (equal 26.0 (node-g (car (expand-node node-marseille-ex6 *travel*))))) ;; 26.0
(assert (equal 'MARSEILLE (node-city (node-parent (car (expand-node node-marseille-ex6 *travel*)))))) ;; MARSEILLE
(assert (equal NIL (node-parent (node-parent (car (expand-node node-marseille-ex6 *travel*)))))) ;; NIL
(assert (equal '(NANCY NANCY) (mapcar #'(lambda (x) (node-city (node-parent x))) (expand-node node-nancy *travel*)))) ;; (NANCY NANCY)
(assert (equal 26.0 (node-g (car (expand-node node-marseille-ex6 *travel*))))) ;; 26.0
(print 'EXERCISE_6_OK)
